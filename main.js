...
case userMessage.startsWith('.deepseek'):
    const deepseekQuery = userMessage.slice(10).trim() || rawText.slice(10).trim();
    if (!deepseekQuery) {
        await sock.sendMessage(chatId, { text: 'Please provide a query.\nExample: .deepseek What is AI?' }, { quoted: message });
        break;
    }
    await deepseekCommand(sock, chatId, message, deepseekQuery);
    break;
